...
対応オンライン証券サービス: E*TRADE from Morgan Stanley
...
- `src/data/murc_ttm_2025.json` には、三菱UFJリサーチ&コンサルティング株式会社が公表するTTM（仲値）データから抽出したUSD/JPYレートが収録されています（243営業日分）
- 土日・祝日はJSONに含まれないため、直前営業日のTTMを自動参照
- TTMデータを更新する場合は、以下のサイトからXLSファイルをダウンロードし、再変換してください
  - https://www.murc-kawasesouba.jp/fx/past_3month.php

> **注意:** 元データ（XLSファイル）は三菱UFJリサーチ&コンサルティング株式会社の著作物であるため、本リポジトリには含まれていません。

## アンインストール

RSU Mate を削除するには、クローンしたディレクトリごと削除してください。

```bash
rm -rf rsu2026
```

RSU Mate はローカルファイル以外に設定やデータを保存しません。ディレクトリの削除のみで完全にアンインストールできます。
